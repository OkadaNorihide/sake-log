...
function toThumbUrl(url: string) { return cloudinaryTransform(url, "f_auto,q_auto,c_fill,w_200,h_200"); }
...
export default function SakeReviewDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews/${encodeURIComponent(id)}`, { cache: "no-store" })
      .then((r) => r.json())
      .then((j: { item?: Review }) => setReview(j.item ?? null))
  ...
  const startEdit = () => {
    if (!review) return;
    setDraftRating(review.rating); setDraftCategory(review.category ?? "不明");
    setDraftTastes([...(review.tastes ?? [])]); setDraftScenes([...(review.scenes ?? [])]);
    setDraftMemo(review.memo ?? ""); setDraftImages([...(review.images ?? [])]);
    setUploadError(""); setEditing(true);
  };
  ...
  const handleImageAdd = async (files: FileList | null) => {
    ...
    try {
      const urls: string[] = [];
      for (const f of Array.from(files).slice(0, remaining)) urls.push(await uploadImage(f));
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/reviews/${encodeURIComponent(review.id)}`, {
        ...
        body: JSON.stringify({ rating: draftRating, category: draftCategory, tastes: draftTastes, scenes: draftScenes, memo: draftMemo.trim(), images: draftImages }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "failed");
      setReview(json.item); setEditing(false);
    } catch (e: unknown) { alert(`保存失敗: ${e instanceof Error ? e.message : "unknown"}`); }
    finally { setSaving(false); }
  };

  if (!id) {
    return <div className="p-6 max-w-2xl mx-auto space-y-4"><div className="rounded-lg border p-6">IDが取得できませんでした</div><Link href="/sake" className="underline text-sm">一覧へ戻る</Link></div>;
  }
          ...
          <div className="rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md p-6 space-y-5 shadow-xl">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-1 text-3xl">
                {[1, 2, 3, 4, 5].map((i) => <button key={i} type="button" onClick={() => setDraftRating(i)} className={i <= draftRating ? "text-yellow-400" : "text-white/20"}>★</button>)}
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs text-white/60">カテゴリ</label>
              <div className="flex flex-wrap gap-2">
                {SAKE_CATEGORIES.map((c) => <button key={c} type="button" onClick={() => setDraftCategory(c)} className={`px-3 py-1 border rounded-full text-sm transition ${draftCategory === c ? "bg-amber-400 text-black border-amber-400" : "border-white/30 hover:bg-white/10"}`}>{c}</button>)}
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs text-white/60">味わい</label>
              <div className="flex flex-wrap gap-2">
                {SAKE_TASTES.map((t) => <button key={t} type="button" onClick={() => setDraftTastes(toggle(t, draftTastes))} className={`px-3 py-1 border rounded-full text-sm transition ${draftTastes.includes(t) ? "bg-white text-black border-white" : "border-white/30 hover:bg-white/10"}`}>#{t}</button>)}
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-xs text-white/60">シーン</label>
              <div className="flex flex-wrap gap-2">
                {SAKE_SCENES.map((s) => <button key={s} type="button" onClick={() => setDraftScenes(toggle(s, draftScenes))} className={`px-3 py-1 border rounded-full text-sm transition ${draftScenes.includes(s) ? "bg-white text-black border-white" : "border-white/30 hover:bg-white/10"}`}>#{s}</button>)}
              </div>
            </div>
            <div className="space-y-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  {draftImages.map((src, idx) => (
                    <div key={idx} className="relative">
                      ...
                      <button type="button" onClick={() => setDraftImages((prev) => prev.filter((_, i) => i !== idx))} className="absolute -top-1 -right-1 bg-black/70 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center hover:bg-red-600 transition">×</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-2">
              ...
              <textarea className="w-full bg-white/10 border border-white/20 rounded-lg p-3 text-sm text-white placeholder-white/40 outline-none focus:ring-2 focus:ring-white/30 resize-none" rows={4} value={draftMemo} onChange={(e) => setDraftMemo(e.target.value)} />
            </div>
            <div className="flex gap-3 pt-1">
              <button onClick={saveEdit} disabled={saving || uploading || draftRating === 0} className="flex-1 bg-white text-black rounded-lg py-2 text-sm font-medium hover:bg-gray-100 transition disabled:opacity-50">{saving ? "保存中..." : "保存する"}</button>
              <button onClick={() => setEditing(false)} disabled={saving} className="flex-1 border border-white/30 rounded-lg py-2 text-sm hover:bg-white/10 transition">キャンセル</button>
            </div>
          </div>
        ) : (
          <>
            <div className="rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md p-6 space-y-4 shadow-xl">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    {review.category && review.category !== "不明" && <span className="text-xs bg-amber-400/20 text-amber-300 border border-amber-400/30 px-2 py-0.5 rounded-full">{review.category}</span>}
                    <div className="text-xs text-white/70">登録日時：{new Date(review.created_at).toLocaleString("ja-JP")}</div>
                  </div>
                </div>
                <div className="text-lg">{"★".repeat(review.rating)}<span className="text-white/30">{"★".repeat(Math.max(0, 5 - review.rating))}</span></div>
              </div>
              ...
              {Array.isArray(review.images) && review.images.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {review.images.slice(0, 3).map((src, idx) => <img key={idx} src={toPreviewUrl(src)} alt={`image-${idx}`} className="w-full aspect-square object-cover rounded border border-white/20 bg-white/10" />)}
                  </div>
                </div>
              )}
              <div className="space-y-1">
              ...
              <div className="pt-2"><LikeButton reviewId={review.id} initialCount={review.likes_count ?? 0} /></div>
            </div>
            <div className="flex gap-3">
              <Link href={`/sake/bottle/${encodeURIComponent(review.name)}`} className="w-1/3 rounded-lg px-4 py-2 text-center bg-white text-black text-sm font-medium">銘柄ページへ</Link>
              <button onClick={startEdit} className="w-1/3 rounded-lg px-4 py-2 bg-white/20 border border-white/30 text-white text-sm hover:bg-white/30 transition">編集する</button>
              <button onClick={async () => {
                ...
                try {
                  ...
                  alert("削除しました"); window.location.href = "/sake";
                } catch { alert("削除に失敗しました"); }
              }} className="w-1/3 bg-red-600 text-white rounded-lg px-4 py-2 text-sm">削除する</button>
            </div>
...
function LikeButton({ reviewId, initialCount }: { reviewId: string; initialCount: number }) {
  ...
  const onLike = async () => {
    ...
    try {
      ...
      const json = (await res.json()) as { likes_count?: number };
  ...
  return (
    <button onClick={onLike} disabled={loading} className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm hover:bg-white/20 disabled:opacity-50">
...
function CommentsSection({ reviewId }: { reviewId: string }) {
  ...
  const childrenByParent = useMemo(() => {
    ...
    for (const c of items) { if (!c.parent_id) continue; const arr = m.get(c.parent_id) ?? []; arr.push(c); m.set(c.parent_id, arr); }
    return m;
  }, [items]);
  ...
  const postComment = async (payload: { parentId?: string | null; body: string }) => {
    const res = await fetch("/api/comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reviewId, parentId: payload.parentId ?? null, authorName, body: payload.body }) });
  ...
  return (
    <div className="rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md p-6 space-y-4 shadow-xl">
      ...
      {loading ? <div className="text-sm text-white/70">読み込み中…</div> : parents.length === 0 ? <div className="text-sm text-white/70">まだコメントがありません。</div> : (
            ...
            return (
              <div key={p.id} className="border border-white/20 rounded-lg p-4 space-y-3 bg-white/95 text-black">
                <div className="flex items-center justify-between gap-3">
                  <div className="text-sm font-medium">{p.author_name || "匿名"}<span className="ml-2 text-xs text-gray-500">{formatJa(p.created_at)}</span></div>
                ...
                {kids.length > 0 && <div className="space-y-3 border-t pt-3">{kids.map((c) => <div key={c.id} className="rounded-md bg-gray-50 p-3"><div className="text-xs text-gray-600"><span className="font-medium">{c.author_name || "匿名"}</span><span className="ml-2">{formatJa(c.created_at)}</span></div><p className="text-sm whitespace-pre-wrap mt-1">{c.body}</p></div>)}</div>}
              </div>
            );
          })}
        </div>
