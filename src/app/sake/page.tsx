...
const SAKE_CATEGORIES = ["純米", "純米吟醸", "純米大吟醸", "吟醸", "大吟醸", "本醸造", "にごり", "古酒", "その他"];
...
function normalizeSearch(s: string): string {
  return s.toLowerCase().replace(/\s+/g, "").replace(/[ァ-ン]/g, (c) => String.fromCharCode(c.charCodeAt(0) - 0x60));
}
...
function buildSummaries(reviews: Review[]): Map<string, BottleSummary> {
  ...
  for (const [name, rs] of byName.entries()) {
    ...
    for (const r of rs) {
      if (r.category && r.category !== "不明") categoryCount.set(r.category, (categoryCount.get(r.category) ?? 0) + 1);
    }
    const topCategory = [...categoryCount.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? null;
    summaries.set(name, {
      name, avgRating: round1(avgRating), reviewCount,
      ...
      category: topCategory, summary: "", hasReviews: true,
    });
  }
  return summaries;
}
...
export default function SakePage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch("/api/reviews?drink_type=sake", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/bottle-infos?drink_type=sake", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/bottles?drink_type=sake", { cache: "no-store" }).then((r) => r.json()),
    ])
      .then(([reviewsJson, infosJson, bottlesJson]) => {
        ...
        for (const { alias, canonical } of (bottlesJson.aliases ?? [])) {
          if (alias && canonical) aMap.set(normalizeSearch(alias as string), canonical as string);
        }
        setAliasMap(aMap);
      })
      .catch(() => { setReviews([]); setBottleInfos([]); })
      .finally(() => setLoading(false));
  }, []);

  const summaries = useMemo(() => {
    const fromReviews = buildSummaries(reviews);
    for (const info of bottleInfos) {
      if (!fromReviews.has(info.name)) {
        fromReviews.set(info.name, {
          name: info.name, avgRating: 0, reviewCount: 0, topTastes: [],
          latestAt: "", thumbUrl: info.hero_image_url ? toThumbUrl(info.hero_image_url) : undefined,
          allTastes: [], allScenes: [], category: info.category, summary: info.summary, hasReviews: false,
        });
      } else {
  ...
  const filtered = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      result = result.filter((s) => normalizeSearch(s.name).includes(nq) || aliasHits.has(s.name));
    }
    ...
    return result;
  }, [summaries, q, filterCategory, filterRating, filterTaste, filterScene, aliasMap]);
      ...
      <div className="relative z-10 p-6 max-w-5xl mx-auto space-y-8">
        ...
        <section className="text-center pt-10 pb-4 space-y-5">
          <h1 className="text-5xl md:text-7xl font-bold tracking-[0.15em] drop-shadow-lg">
            <Link href="/" className="hover:opacity-80 transition">Ichigo LOG</Link>
          </h1>
          ...
          <div className="pt-2 flex items-center justify-center gap-3 flex-wrap">
            <Link href="/sake/register" className="inline-block bg-white text-black rounded-full px-8 py-3 font-medium hover:bg-gray-100 transition shadow-lg tracking-wide">
              ＋ 日本酒を登録する
            </Link>
            <Link href="/" className="inline-block border border-white/40 text-white rounded-full px-6 py-3 text-sm hover:bg-white/10 transition tracking-wide">
              🥃 ウィスキーサイトへ
            </Link>
          </div>
        </section>
        ...
        <section className="bg-white/10 backdrop-blur-md rounded-2xl p-5 space-y-5">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            />
            <select className="bg-white/20 border border-white/20 text-white p-3 rounded-lg md:w-48" value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)}>
          ...
          <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {SAKE_CATEGORIES.map((c) => (
                  <button key={c} onClick={() => setFilterCategory(filterCategory === c ? null : c)}
                    className={`px-3 py-1 rounded-full text-sm border transition ${filterCategory === c ? "bg-amber-400 text-black border-amber-400" : "border-white/25 hover:bg-white/10"}`}>
                    {c}
                  </button>
                ))}
              </div>
            ...
            <button onClick={() => setShowDetailFilter((v) => !v)} className="flex items-center gap-1 text-xs text-white/50 hover:text-white/80 transition">
            ...
            {showDetailFilter && (
              <div className="space-y-3 pt-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[4.5, 4, 3.5, 3].map((r) => (
                      <button key={r} onClick={() => setFilterRating(filterRating === r ? null : r)}
                        className={`px-3 py-1 rounded-full text-sm border transition ${filterRating === r ? "bg-yellow-400 text-black border-yellow-400" : "border-white/25 hover:bg-white/10"}`}>
                        ★{r}以上
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SAKE_TASTES.map((t) => (
                      <button key={t} onClick={() => setFilterTaste(filterTaste === t ? null : t)}
                        className={`px-3 py-1 rounded-full text-sm border transition ${filterTaste === t ? "bg-white text-black border-white" : "border-white/25 hover:bg-white/10"}`}>
                        #{t}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SAKE_SCENES.map((s) => (
                      <button key={s} onClick={() => setFilterScene(filterScene === s ? null : s)}
                        className={`px-3 py-1 rounded-full text-sm border transition ${filterScene === s ? "bg-white text-black border-white" : "border-white/25 hover:bg-white/10"}`}>
                        #{s}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2 pt-1 border-t border-white/10">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {SAKE_TASTES.map((t) => {
                        ...
                        return (
                          <button key={t} onClick={() => toggleMapAttr(t)}
                            className={`px-3 py-1 rounded-full text-sm border transition ${idx === 0 ? "bg-sky-400 text-black border-sky-400" : idx === 1 ? "bg-emerald-400 text-black border-emerald-400" : "border-white/25 hover:bg-white/10"}`}>
                            {idx === 0 ? "X " : idx === 1 ? "Y " : ""}{t}
                          </button>
                        );
                      })}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {SAKE_SCENES.map((s) => {
                        ...
                        return (
                          <button key={s} onClick={() => toggleMapAttr(s)}
                            className={`px-3 py-1 rounded-full text-sm border transition ${idx === 0 ? "bg-sky-400 text-black border-sky-400" : idx === 1 ? "bg-emerald-400 text-black border-emerald-400" : "border-white/25 hover:bg-white/10"}`}>
                            {idx === 0 ? "X " : idx === 1 ? "Y " : ""}{s}
                          </button>
                        );
                  ...
                  {mapAttrs.length < 2 && mapAttrs.length > 0 && <p className="text-xs text-white/40">あと {2 - mapAttrs.length} つ選んでください</p>}
                  {mapAttrs.length === 2 && (() => {
                    const SVG_W = 500, SVG_H = 400, PL = 70, PR = 20, PT = 20, PB = 52;
                    ...
                    return (
                      <div className="mt-2 bg-white/5 rounded-xl p-2">
                        {mapPoints.length === 0 ? (
                          ...
                          <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full" style={{ maxHeight: 400 }}>
                            ...
                            {mapPoints.map((pt) => {
                              ...
                              return (
                                <a key={pt.name} href={`/sake/bottle/${encodeURIComponent(pt.name)}`}>
                        ...
                        <p className="text-xs text-white/30 text-center mt-1">クリックで銘柄詳細へ</p>
                      </div>
                    );
                  })()}
                  {mapAttrs.length > 0 && <button onClick={() => setMapAttrs([])} className="text-xs text-white/40 underline hover:text-white/70 transition">選択をリセット</button>}
                </div>
              </div>
            )}

            {hasFilter && (
              <button onClick={() => { setFilterCategory(null); setFilterRating(null); setFilterTaste(null); setFilterScene(null); }} className="text-xs text-white/50 underline hover:text-white/80 transition">
                フィルターをリセット
              </button>
            )}
          </div>
            ...
            <section className="space-y-3">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
                {recentVisible.map((r) => {
                  ...
                  return (
                    <Link key={r.id} href={`/sake/alcohol/${encodeURIComponent(r.id)}`}
                      ...
                      {thumb ? <img src={thumb} alt={r.name} className="w-full h-28 object-cover" /> : <div className="w-full h-28 bg-white/5 flex items-center justify-center text-xs text-white/30">no photo</div>}
              ...
              {recentRest.length > 0 && (
                <>
                      ...
                      {recentRest.map((r) => {
                        ...
                        return (
                          <Link key={r.id} href={`/sake/alcohol/${encodeURIComponent(r.id)}`}
                            ...
                            {thumb ? <img src={thumb} alt={r.name} className="w-full h-24 object-cover" /> : <div className="w-full h-24 bg-white/5 flex items-center justify-center text-xs text-white/30">no photo</div>}
                            <div className="p-2 space-y-1">
                              <div className="text-xs font-semibold line-clamp-1">{r.name}</div>
                              <div className="text-xs text-amber-300">{"★".repeat(r.rating)}</div>
                            </div>
                          </Link>
                        );
                      })}
                  ...
                  <div className="text-center">
                    <button onClick={() => setShowAllRecent((v) => !v)} className="px-6 py-2 bg-white/10 border border-white/20 rounded-full text-xs hover:bg-white/20 transition">
        ...
        {/* 銘柄一覧 */}
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-semibold tracking-wide">{hasFilter || q.trim() ? "検索結果" : "銘柄一覧"}</h2>
          <div className="flex-1 h-px bg-white/20" />
          {!loading && <span className="text-xs text-white/40 shrink-0">{sorted.length}件</span>}
        </div>
          ...
          <>
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {sorted.slice(0, visibleCount).map((s) => (
                <Link key={s.name} href={`/sake/bottle/${encodeURIComponent(s.name)}`}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        {s.category && <span className="text-xs bg-amber-400/20 text-amber-300 border border-amber-400/30 px-2 py-0.5 rounded-full">{s.category}</span>}
                      </div>
                    </div>
                    <div className="text-right text-sm shrink-0">
                      {s.hasReviews ? (<>★ {s.avgRating.toFixed(1)}<div className="text-xs text-gray-300">{s.reviewCount}件</div></>) : (<div className="text-xs text-white/40">レビューなし</div>)}
                    </div>
                  </div>
                  {s.topTastes.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {s.topTastes.map((t) => <span key={t.label} className="text-xs bg-white/20 px-2 py-1 rounded-full">#{t.label} · {t.count}</span>)}
                    </div>
                  )}
                  {s.hasReviews && <div className="text-xs text-gray-300">最終レビュー：{new Date(s.latestAt).toLocaleString("ja-JP")}</div>}
                </Link>
              ))}
            </section>
            {visibleCount < sorted.length && (
              <div className="text-center pt-2">
                <button onClick={() => setVisibleCount((n) => n + 25)} className="px-8 py-3 bg-white/10 border border-white/20 rounded-full text-sm hover:bg-white/20 transition">
