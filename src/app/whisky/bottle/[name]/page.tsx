  ...
  if (!decodedName) {
    return (
      <div className="p-6 max-w-3xl mx-auto space-y-4">
        ...
        <Link href="/whisky" className="underline text-sm">一覧へ戻る</Link>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen text-white">
      ...
      <div className="relative z-10 p-6 max-w-5xl mx-auto space-y-6">
        <header className="flex items-start justify-between gap-4">
          ...
          <Link href="/whisky" className="inline-flex items-center bg-white text-black rounded-lg px-4 py-2 font-medium shadow hover:bg-gray-100 transition">
            一覧へ戻る
          </Link>
        </header>
        ...
        <section className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 text-white/80">
              まだレビューがありません。
              <div className="mt-3"><Link href="/whisky/register" className="underline">＋ お酒を登録</Link></div>
            </div>
          ) : (
            reviews.map((r) => (
              <div key={r.id} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 space-y-3 shadow-xl">
                ...
                {Array.isArray(r.images) && r.images.length > 0 && (
                  <div className="flex gap-2 flex-wrap">
                    {r.images.slice(0, 3).map((src, idx) => (
                      <Link key={idx} href={`/whisky/alcohol/${encodeURIComponent(r.id)}`}>
                ...
                <div>
                  <Link href={`/whisky/alcohol/${encodeURIComponent(r.id)}`} className="text-sm underline text-white/80">
                    このレビューの詳細を見る
                  </Link>
                </div>
              </div>
        ...
        <div className="flex gap-3">
          <Link href={`/whisky/register?name=${encodeURIComponent(decodedName)}`} className="w-1/2 bg-white text-black rounded-lg px-4 py-3 text-center font-medium shadow hover:bg-gray-100 transition">
            追加で登録
          </Link>
          <Link href="/whisky" className="w-1/2 bg-black/60 border border-white/20 rounded-lg px-4 py-3 text-center font-medium hover:bg-black/70 transition">
            一覧へ
          </Link>
        </div>
      </div>
