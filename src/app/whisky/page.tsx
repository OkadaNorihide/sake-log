    ...
    const fetchAll = async () => {
      ...
      try {
        const [reviewsRes, infosRes, bottlesRes] = await Promise.all([
          ...
          fetch("/api/bottle-infos?drink_type=whisky", { cache: "no-store" }),
          fetch("/api/bottles?drink_type=whisky", { cache: "no-store" }),
        ]);
      ...
      <div className="relative z-10 p-6 max-w-5xl mx-auto space-y-8">
        ...
        <section className="text-center pt-10 pb-4 space-y-5">
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            ...
            <Link
              href="/"
              className="inline-block border border-white/40 text-white rounded-full px-6 py-3 text-sm hover:bg-white/10 transition tracking-wide"
            >
              ← ホームへ
            </Link>
          </div>
                          ...
                          <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full" style={{ maxHeight: 400 }}>
                            ...
                            {(() => {
                              ...
                              return mapPoints.map((pt) => {
                                ...
                                return (
                                  <a key={pt.name} href={`/whisky/bottle/${encodeURIComponent(pt.name)}`}>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
                {recentVisible.map((r) => {
                  ...
                  return (
                    <Link
                      key={r.id}
                      href={`/whisky/alcohol/${encodeURIComponent(r.id)}`}
          ...
          <>
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {sorted.slice(0, visibleCount).map((s) => (
                <Link
                  key={s.name}
                  href={`/whisky/bottle/${encodeURIComponent(s.name)}`}
